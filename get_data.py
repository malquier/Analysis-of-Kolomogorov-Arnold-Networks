...
class OptionPricingDataset(Dataset):
    def __init__(self, ticker="^STOXX50E", start_date="2015-01-01", end_date=None, 
                 strike=3500.0, vol=0.2, rate=0.05, maturity=1.0):
        
        self.df = yf.download(ticker, start=start_date, end=end_date, progress=False, multi_level_index=False)
    ...
    def call_BS(self, s0):
        ...
        d1 = (np.log(s0 / self.strike) + (self.rate + 0.5 * self.vol**2) * self.maturity) / (self.vol * np.sqrt(self.maturity))
        d2 = d1 - self.vol * np.sqrt(self.maturity)
        
        t_d1 = torch.tensor(d1, dtype=torch.float32)
        t_d2 = torch.tensor(d2, dtype=torch.float32)
        
        price = s0 * torch.special.ndtr(t_d1) - self.strike * np.exp(-self.rate * self.maturity) * torch.special.ndtr(t_d2)
...
if __name__ == "__main__":
    ...
    X_train, y_train, X_test, y_test = get_train_test_datasets(full_dataset, split_ratio=0.8)
